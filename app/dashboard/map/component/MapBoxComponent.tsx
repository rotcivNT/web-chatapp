...
const MapboxComponent = () => {
  ...
  const handleOnClick = async () => {
    ...
    const dataStranger = await userAPI.getUserByPhone(
      `/user/get-user/${idStranger}`
    );
  ...
  useEffect(() => {
    mapboxgl.accessToken =
      "pk.eyJ1IjoidHJhbmxvYzJrMyIsImEiOiJjbHZxYnR2bDYwYmppMmpwNnRnemlhaHA5In0.Fn9lSoYFUZ96simIJs9s4g"; // Thay thế bằng access token của bạn từ Mapbox
    navigator.geolocation.getCurrentPosition((position) => {
      if (mapContainer.current) {
        ...
        const root = ReactDOMClient.createRoot(markerElement as HTMLElement);
        ...
        map.on("load", () => {
          ...
          map.loadImage(
            ...
            function (error, image) {
              ...
              map.addImage("my-marker", image as any);
            }
          );
          ...
          map.on("click", "unclustered-point", async function (e) {
            if (e.features) {
              ...
              const dataUser = await userAPI.getUserByPhone(
                `/user/get-user/${id}`
              );
